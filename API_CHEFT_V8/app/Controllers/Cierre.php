	...
	public function update()
	{
		if ($this->request->isAJAX()) {
			...
			$dataModel = [
				...
				'Total_Semana' => 0,  // Mantener inicializado para ser actualizado después
				'Total_Mes' => 0,      // Mantener inicializado para ser actualizado después
				'idUsuario_fk' => session()->get('user_id'),			
				'create_at' => date("Y-m-d H:i:s"),			
				'update_at' => date("Y-m-d H:i:s"),			
			];

			if ($this->cierreModel->update($id, array_merge($dataModel, ['Total_Semana' => 0, 'Total_Mes' => 0]))) { 
				$data['message'] = 'success';
				$data['response'] = ResponseInterface::HTTP_OK;
				$data['data'] = array_merge($dataModel, ['idCierre' => 0]); 
				$data['csrf'] = csrf_hash();
			} else {
